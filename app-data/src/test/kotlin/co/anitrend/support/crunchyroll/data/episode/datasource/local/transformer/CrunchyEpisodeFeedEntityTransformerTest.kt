...
class CrunchyEpisodeFeedEntityTransformerTest {

    private val locale by lazy { Locale("English", "United Kingdom") }

    @Test
    fun `given list of thumbnails is highest quality true`() {
        val case = listOf(
            EpisodeThumbnailModel(
                url = "",
                width = 100,
                height = 56
            ),
        ...
        val actual = EpisodeFeedHelper.highestQuality(case)

        val expected = EpisodeThumbnailModel(
            url = "",
            width = 640,
            height = 360
        )
    ...
    fun `given duration in seconds assert string representation is correct`() {
        ...
        val actual = EpisodeFeedHelper.durationFormatted(case)
    ...
    fun `given a string of locale for available languages assert produces list of platform locales`() {
        val case = "en - us,en - uk,es - la,es - es,pt - br,ar - me,it - it,de - de,ru - ru"

        val actual = EpisodeFeedHelper.getSubtitles(
            case, locale
        )

        val expected = listOf(
            "enUS", "enUK", "esLA", "esES", "ptBR", "arME", "itIT", "deDE", "ruRU"
        )
    ...
    fun `given string of countries assert maps correctly`() {
        ...
        val case = CrunchyEpisodeRestrictionModel(
        ...
        val actual = EpisodeFeedHelper.isAllowed(case, locale)
