...
package co.anitrend.support.crunchyroll.data.stream.transformer
...
internal object MediaStreamTransformer : ISupportMapperHelper<CrunchyStreamInfoModel?, List<MediaStream>?> {
     ...
     */
    override fun transform(source: CrunchyStreamInfoModel?): List<MediaStream>? {
        return source?.stream_data?.let { crunchyStreamDataModel ->
            crunchyStreamDataModel.streams.map {
                MediaStream(
                    subTitleLanguage = crunchyStreamDataModel.hardsub_lang,
                    audioLanguage = crunchyStreamDataModel.audio_lang,
                    format = crunchyStreamDataModel.format,
                    quality = it.quality,
                    url = it.url,
