...
class RssNewsAdapter(
    ...
    private val clickListener: ItemClickListener<CrunchyRssNews>
) : SupportViewAdapter<CrunchyRssNews>(presenter), KoinComponent {
    ...
    ): SupportViewHolder<CrunchyRssNews> {
    ...
    inner class NewsRssViewHolder(
    ...
    ): SupportViewHolder<CrunchyRssNews>(binding.root) {
         ...
         */
        override fun invoke(model: CrunchyRssNews?) {
            with (binding) {
                entity = model
                markwon.setMarkdown(
