...
class CrunchyMediaListRepository(
    private val useCase: CrunchyMediaListUseCase
) : SupportRepository<PagedList<CrunchyMedia>, CrunchyMediaListUseCase.Payload>() {
    /**
     * Handles dispatching of network requests to a background thread
     *
     * @param subject subject to apply business rules
     */
    override fun invoke(subject: CrunchyMediaListUseCase.Payload) =
        useCase(
            param = subject
        )
}